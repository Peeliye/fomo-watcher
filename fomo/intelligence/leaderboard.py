...
def _public_entry(item:dict[str,Any],position:int,window:str)->dict[str,Any]:
    ...
    pnl=round(_number(item.get(field,item.get("pnlUsd"))),6)
    return {"userId":uid,"rank":int(_number(item.get("rank")) or position),"pnlUsd":pnl,"pnl24hUsd":pnl,"pnlSource":"fomo_rolling_24h","verificationStatus":"fomo_reported","userHandle":str(item.get("userHandle") or "unknown"),"displayName":str(item.get("displayName") or ""),"address":str(item.get("address") or ""),"evmAddress":str(item.get("evmAddress") or ""),"followers":int(_number(item.get("followers"))),"numTrades":int(_number(item.get("numTrades"))),"totalVolume":round(_number(item.get("totalVolume")),6),"totalHoldings":int(_number(item.get("totalHoldings"))),"clan":item.get("clan") if isinstance(item.get("clan"),dict) else None,"topHoldings":item.get("topHoldings") if isinstance(item.get("topHoldings"),list) else []}
...
class LeaderboardArchive:
    ...
    def overview(self,day:str|None=None)->dict[str,Any]:
        ...
        return {"cycleDate":day,"timezone":str(self.timezone),"progress":f"{len(success)}/24","successfulSnapshots":len(success),"failedSnapshots":sum(r["status"]=="failed" for r in rows),"currentRanked":int(latest["source_count"]) if latest else 0,"uniqueParticipants":int(unique),"lastSnapshot":latest["captured_at"] if latest else None,"pnlMetric":"fomo_rolling_24h","participantSort":"current_24h_pnl_then_last_observed","hours":[{"hourIndex":r["hour_index"],"status":r["status"],"capturedAt":r["captured_at"],"count":r["source_count"]} for r in rows]}
    def _records(self,sid:int)->list[dict[str,Any]]:
        captured=self.db.execute("SELECT captured_at FROM snapshots WHERE snapshot_id=?",(sid,)).fetchone();as_of=captured[0] if captured else None
        records=[]
        for row in self.db.execute("SELECT * FROM ranking_records WHERE snapshot_id=? ORDER BY rank",(sid,)):
            item=json.loads(row["payload_json"]);item.update({"pnl24hUsd":item.get("pnl24hUsd",item.get("pnlUsd")),"pnlSource":"fomo_rolling_24h","asOf":as_of,"freshness":"snapshot","verificationStatus":"fomo_reported"});records.append(item)
        return records
    ...
    def participants(self,day:str|None=None)->list[dict[str,Any]]:
        ...
        for row in rows:
            ...
            if x is None:x={"userId":row["user_id"],"userHandle":row["user_handle"],"displayName":row["display_name"],"address":row["address"],"evmAddress":row["evm_address"],"firstSeen":row["captured_at"],"firstPnl24hUsd":row["pnl_usd"],"firstPnl":row["pnl_usd"],"firstRank":row["rank"],"maxPnl24hUsd":row["pnl_usd"],"minPnl24hUsd":row["pnl_usd"],"bestRank":row["rank"],"appearCount":0};agg[row["user_id"]]=x
            x.update({"lastObservedAt":row["captured_at"],"lastSeen":row["captured_at"],"latestRank":row["rank"],"latestRankedPnl24hUsd":row["pnl_usd"],"latestRankedPnl":row["pnl_usd"],"userHandle":row["user_handle"],"isCurrentRanked":int(row["snapshot_id"])==int(row["current_snapshot"])});x["appearCount"]+=1;x["bestRank"]=min(x["bestRank"],row["rank"]);x["maxPnl24hUsd"]=max(x["maxPnl24hUsd"],row["pnl_usd"]);x["minPnl24hUsd"]=min(x["minPnl24hUsd"],row["pnl_usd"])
        for x in agg.values():x["rankedPnl24hChangeUsd"]=round(x["latestRankedPnl24hUsd"]-x["firstPnl24hUsd"],6);x["currentPnl24hUsd"]=x["latestRankedPnl24hUsd"] if x["isCurrentRanked"] else None;x["currentValueAvailable"]=x["isCurrentRanked"];x["appearances"]=x["appearCount"];x["lastRank"]=x["latestRank"];x["presentInLatest"]=x["isCurrentRanked"];x["pnlSource"]="fomo_rolling_24h";x["verificationStatus"]="fomo_reported"
        return sorted(agg.values(),key=lambda x:(not x["isCurrentRanked"],-(x["currentPnl24hUsd"] if x["currentPnl24hUsd"] is not None else float("-inf")),-datetime.fromisoformat(str(x["lastObservedAt"]).replace("Z","+00:00")).timestamp()))
    def participant_detail(self,day:str,user_id:str)->dict[str,Any]:
        ...
        return {"cycleDate":day,"userId":user_id,"pnlMetric":"fomo_rolling_24h","history":[{"hourIndex":h,"rank":by[h]["rank"] if h in by else None,"pnlUsd":by[h]["pnl_usd"] if h in by else None,"pnl24hUsd":by[h]["pnl_usd"] if h in by else None,"asOf":by[h]["captured_at"] if h in by else None,"status":"ranked" if h in by and by[h]["rank"] is not None else "not_ranked"} for h in range(24)]}
