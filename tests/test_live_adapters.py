...
class LiveAdapterTests(unittest.TestCase):
    def test_journal_v4_migration_has_verified_rollback_backup(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(journal.db.execute("PRAGMA user_version").fetchone()[0], 7)
            self.assertEqual(journal.db.execute("SELECT value FROM preserved_history").fetchone()[0], "keep-me")
            journal.close()
            backups = list((Path(directory) / "backups").glob("execution.pre-v7.from-v4.*.sqlite3"))
    ...
    def test_v5_nonce_audit_migration_preserves_old_leases_in_backup(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            journal.close()
            backups = list((Path(directory) / "backups").glob("execution.pre-v7.from-v5.*.sqlite3"))
            self.assertEqual(len(backups), 1)
            backup = sqlite3.connect(backups[0])
            self.assertEqual(backup.execute("PRAGMA integrity_check").fetchone()[0], "ok")
